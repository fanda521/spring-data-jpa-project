 ...
 */
@Repository
public interface ComputerRepository extends JpaRepository<Computer, ComputerPK> {
    //根据ip
    List<Computer> findByComputerPK_Ip(String ip);

    //根据 ownerId
    List<Computer> findByComputerPK_OwnerId(String ownerId);
    ...
    @Modifying
    //@Transactional
    @Query("update Computer set brandName =:brandName where computerPK.ip =:ip")
    int updateComputerByIp(@Param("brandName") String name,@Param("ip") String ip);

    //尝试使用本地sql
    @Modifying
    @Query(value = "update t_computer set t_brand_name =:brandName where t_ip =:ip",nativeQuery = true)
    int updateComputerByIpNative(@Param("brandName") String name,@Param("ip") String ip);

    int deleteByComputerPK_OwnerId(String ownerId);
